...
export class ControleLivrosService {
  ...
  incluir(novoLivro: Livro): void {
    if (this.livrosMock.length > 0) {
      const maiorCod = Math.max(
        ...this.livrosMock.map((livro) => livro.codigo)
      );
  ...
  excluir(codigoLivro: number): void {
    const index = this.livrosMock.findIndex((fi) => fi.codigo === codigoLivro);
    if (index !== -1) {
      this.livrosMock.splice(index, 1);
    }
  }
}
